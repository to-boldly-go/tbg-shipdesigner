...
Promise.all([readFile(args.parts, 'utf8').then(Papa.parsePromise),
	...
	.then(([parts, frames, design]) => {
		...
		// console.log(se_design);
		console.log(se_design.combat);
		console.log(se_design.raw_combat);
		console.log(se_design.subsystems[0].components.map((comp) => [comp.name, comp.combat]));
	});
