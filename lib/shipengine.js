...
class DesignComponent {
	...
	attribute(name) {
		if (!this.part_def) {
			if (this.db.part_schema.get(name).edit_type === 'number') {
				return 0;
			} else {
				return '';
			}
		}
		return this.part_def[name];
	}
	...
	get tech_year() {
		return this.attribute('Year Available (SF)');
	};
...
class DesignSubsystem {
	...
	frame_attribute(name) {
		if (!this.sub_frame_def) {
			if (this.db.frame_schema.get(name).edit_type === 'number') {
				return 0;
			} else {
				return '';
			}
		}
		return this.sub_frame_def[name];
	}
	...
	get is_loaded() {
		return this.is_valid_frame && this.components.every(comp => comp.is_loaded);
	}

	get tech_year_frame() {
		return this.frame_attribute('Year Available (SF)');
	};
	...
	get build_time() {
		...
		// frames list
		return from_frac(this.frame_attribute('Build Time'));
	};
...
class Module {
	...
	attribute(name) {
		if (!this.module_def) {
			if (this.db.module_schema.get(name).edit_type === 'number') {
				return 0;
			} else {
				return '';
			}
		}
		return this.module_def[name];
	}

	get is_loaded() {
		return !!this.module_def;
	}
	...
	get valid_variants() {
		return this.db.find_modules(this.attribute('Type'));
	};
	...
	get build_time() {
		...
		return from_frac(this.attribute('Build Time'));
	};
...
class Design {
	...
	frame_attribute(name) {
		...
		return this.princ_frame_def[name];
	}

	get is_valid_frame() {
		return !!this.princ_frame_def;
	}
