...
class DesignComponent {
	...
	get is_loaded() {
		return this.part_def != undefined;
	};
	...
	get is_invalid() {
		return (!this.is_loaded) || this.quantity_error || this.valid_part_error;
	};

	get errors() {
		return [
			this.quantity_error
				? this.name + ': invalid quantity ' + this.quantity.toString()
				: false,
			this.valid_part_error
				? this.name + ': invalid part ' + this.part
				: false,
		].filter((elem) => elem);
	};

	test_quantity_valid(value) {
		return _.some(this.valid_quantities, (elem) => (elem === value));
	};

	get quantity_error() {
		if (this.is_quantity_configurable) {
			return !(this.valid_quantities.includes(this.quantity));
		} else {
			return false;
		};
	};

	get valid_part_error() {
		return !(this.valid_parts
				 .map((part) => part['Name'])
				 .includes(this.part));
	};

	get tech_year_warning() {
		return this.tech_year == this.subsystem.design.tech_year;
	};
	...
	set quantity(value) {
		if (this.is_quantity_configurable) {
			this.set_quantity_no_magic(value);
			if (value === 0) {
	...
	set_quantity_no_magic(value) {
		this.quantity_calcs.set(parseInt(value));
	};

	get is_quantity_configurable() {
		return !this.is_no_part && this.quantity_calcs.is_configurable;
	};
...
class DesignSubsystem {
	...
	get tech_year_max() {
		const years = _.chain(this.components)
			  ...
			  .filter((n) => n)
			  .value();
		return _.max([...years, this.tech_year_frame]);
	};
	...
	get is_invalid() {
		return this.component_invalid_error || this.invalid_frame_error || this.weight_error;
	};

	get errors() {
		return [
			this.weight_error
				? this.name + ': over weight ' + this.weight_internal.toString() + "/" + this.weight_cap.toString()
				: false,
			this.invalid_frame_error
				? this.name + ': invalid subframe ' + this.sub_frame + ' in ' + this.design.princ_frame
				: false,
			..._.flatMap(this.components, (comp) => comp.errors),
		].filter((elem) => elem);
	};

	get invalid_frame_error() {
	...
	get weight_error() {
		return this.weight_internal > this.weight_cap;
	};

	get component_invalid_error() {
		return this.components
			.map((comp) => comp.is_invalid)
			.reduce((result, v) => result || v, false);
	};
	...
	get setting_safety_performance() {
		...
		return -this.get_setting('Safety/Performance')
			* this.component('Warp Core Type').effect
...
class Design {
	constructor(db, design_json, wrong_db = false) {
		...
		this.princ_frame = this.json['Principal Frame'];
		this.princ_frame_def = this.db.find_frame(this.princ_frame);
	...
	get errors() {
		return [
			this.weight_error
				? 'Design over weight ' + this.weight_internal.toString() + "/" + this.frame_max_size_raw.toString()
				: false,
			this.power_error
				? 'Design over power ' + this.cost_power.toString() + "/" + this.power_generation.toString()
				: false,
			..._.flatMap(this.subsystems, (ss) => ss.errors),
		].filter((elem) => elem);
	};
	...
	get is_invalid() {
		return this.subsystem_invalid_error || this.weight_error || this.power_error;
	};

	get power_error() {
		return this.cost_power > this.power_generation;
	};

	get weight_error() {
		return this.weight_internal > this.frame_max_size_raw;
	};

	get subsystem_invalid_error() {
		return this.subsystems
			.map((ss) => ss.is_invalid)
			.reduce((result, v) => result || v, false);
	};
	...
	get tech_year_max() {
		const years = _.chain(this.subsystems)
			  ...
			  .filter((n) => n)
			  .value();
		return _.max([...years, this.frame_year_frame]);
	};
